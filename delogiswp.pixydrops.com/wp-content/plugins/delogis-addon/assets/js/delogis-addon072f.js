(function ($) {
  ...
  $(window).on("elementor/frontend/init", function () {
    ...
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/delogis-faq.default",
      WidgetFaqHandler
    );
    ...
    // login
    $("#delogis-login").submit(function (event) {
      ...
      $.ajax({
          type: "POST",
          url: delogis_login_object.ajaxurl,
          data: login,
          beforeSend: function () {
              // setting a timeout
          ...
          success: function (data) {
              ...
              if (data.status == 2) {
                  ...
                  window.location.href = delogis_login_object.login_redirect_url;
              } else if (data.status == 1) {
                  $(".login-result").html(data.message).addClass('alert alert-warning');
              } else {
                  $(".login-result").html(delogis_login_object.message).addClass('alert alert-warning');
              }
          }
      });
  }); //end login
  ...
  // register
  $("#delogis-registration").submit(function (event) {
      ...
      $.ajax({
          ...
          success: function (data) {
              ...
              if (data.status == 2) {
                  ...
                  window.location.href = delogis_login_object.registration_redirect_url;
              } else {
